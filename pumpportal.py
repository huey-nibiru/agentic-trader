...
from executor import _log_trade, _realized_pnl, _with_extras
...
class PumpPortalDiscovery:
    def __init__(self, dexscreener_client, chain_id: str = "solana"):
        ...
        self.last_checked = {}  # mint -> last Dexscreener check time
        self.seen_pairs = set()
        ...
        while True:
            try:
                async with websockets.connect(WS_URL) as ws:
                    ...
                    print("[pumpportal] connected, subscribed to new token events", flush=True)
    ...
    def status_snapshot(self) -> dict:
        """Pending-pool size and lifetime token count for the heartbeat line."""
        with self._lock:
            return {"pending": len(self.pending), "tokens_seen": self.tokens_seen_count}

    def check_matured_candidates(self, cfg) -> list[dict]:
        """
        Recheck pending PumpPortal mints against Dexscreener until they pass
        entry filters or age past MAX_PAIR_AGE_SECONDS (24h). A mint is not
        discarded on the first failed liquidity/volume check.
        """
        ...
        candidates = []
        rejection_counts = {}
        recheck_after = getattr(cfg, "DISCOVERY_RECHECK_SECONDS", 60)
        max_checks = getattr(cfg, "DISCOVERY_MAX_CHECKS_PER_PASS", 20)
        ...
        with self._lock:
            ...
            for mint in list(self.pending.keys()):
                if now - self.pending[mint] > cfg.MAX_PAIR_AGE_SECONDS:
                    ...
                    self.last_checked.pop(mint, None)
                    dropped += 1
            due = [
                m for m, created in self.pending.items()
                if now - created >= cfg.MIN_PAIR_AGE_SECONDS
                and now - self.last_checked.get(m, 0) >= recheck_after
            ]
            # never-checked first, then oldest creates
            due.sort(key=lambda m: (self.last_checked.get(m, 0) > 0, self.pending[m]))
            mints_to_check = due[:max_checks]
            pending_count = len(self.pending)
            tokens_seen = self.tokens_seen_count

        for mint in mints_to_check:
            with self._lock:
                self.last_checked[mint] = now
            try:
                pairs = self.client.get_token_pairs(self.chain_id, mint)
            except requests.RequestException:
                rejection_counts["fetch error"] = rejection_counts.get("fetch error", 0) + 1
                continue
            if not pairs:
        ...
        status = (f"[pumpportal/scan] pending={pending_count} "
                  ...
                  f"total_tokens_seen={tokens_seen}")
        if dropped:
...
class PumpPortalExecutor:
    ...
    def buy(self, symbol: str, mint: str, sol_amount: float, price: float,
        ...
        fill = _with_extras({
            ...
            "side": "BUY", "state": "open", "usd": sol_amount * price,
            "price": price, "tokens": tokens_est, "ts": time.time(),
            "mode": "LIVE-PUMPPORTAL", "signature": sig,
            "pnl": _realized_pnl("BUY", tokens_est, price),
        }, extras)
        _log_trade(fill)
        print(f"[LIVE/PumpPortal] BUY {symbol} {sol_amount} SOL -> tx {sig}")
        return fill

    def sell(self, symbol: str, mint: str, tokens: float, price: float,
             entry_price: float = None, extras: dict = None) -> dict:
        raw_tx = self._get_unsigned_tx("sell", mint, tokens, denominated_in_sol=False)
        sig = self._sign_and_send(raw_tx)
        fill = _with_extras({
            "symbol": symbol, "contract_address": mint,
            "side": "SELL", "state": "closed", "usd": tokens * price,
